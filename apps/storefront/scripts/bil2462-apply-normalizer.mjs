#!/usr/bin/env node
// BIL-2462: apply the v2 studio-look normalizer (bil2462-studio-normalize.mjs)
// to every live Medusa product thumbnail, upload the result, and PATCH
// product.thumbnail — the step that was still missing after the normalizer
// itself was written and validated (see docs/design/RESHOOT-LIST-2026-08-14.md).
//
// Same shape as bil2455-batch-normalize-thumbnails.mjs, just pointed at the
// v2 script (border-seeded flood-fill repaint instead of crop-only).
//
// Usage:
//   MEDUSA_ADMIN_EMAIL=... MEDUSA_ADMIN_PASSWORD=... \
//     node apps/storefront/scripts/bil2462-apply-normalizer.mjs
...
const WORK = path.resolve(REPO_ROOT, ".paperclip-scratch/bil2462-apply");
...
function runNormalizer(inFile, outFile) {
  return new Promise((resolve, reject) => {
    const p = spawn(process.execPath, [
      path.join(HERE, "bil2462-studio-normalize.mjs"),
...
for (const p of products) {
  ...
  const safe = p.id + ext; // fixed short name — the live thumbnail's own
  // basename grows every re-upload (upload service prefixes a timestamp and
  // keeps the old name), which blows past Windows' ~260-char path limit
  // after a couple of normalizer passes.
  ...
  try {
    ...
    results.push({ id: p.id, title: p.title, status: "UPDATED", mode, oldUrl: url, newUrl });
    console.log(`OK  [${mode}] ${p.title} -> ${newUrl}`);
  } catch (e) {
...
console.log("  updated (segmented):", results.filter(r => r.status === "UPDATED" && r.mode === "segmented").length);
console.log("  updated (fallback):", results.filter(r => r.status === "UPDATED" && r.mode === "fallback").length);
